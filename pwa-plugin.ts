...
// visibly separate home-screen tiles instead of three identical "Time"
...
function channelName(base: string): { name: string; short_name: string } {
  if (base === "/preview/")
    return { name: "Time (preview)", short_name: "Time pre" };
  if (base === "/branch/")
    return { name: "Time (branch)", short_name: "Time br" };
  return {
    name: "Time",
    short_name: "Time",
  };
...
export function buildManifest(base: string): string {
  ...
  const manifest = {
    ...
    short_name,
    description:
      "A privacy-first time report PWA: start and stop working, log breaks and work categories as they happen or afterwards, and read your hours back per project as charts. Local-only or cloud-synced (Dropbox, Dropbox).",
    id: base,
    start_url: base,
    scope: base,
    display: "standalone",
...
// GENERATED — do not edit. Emitted by pwa-plugin.ts for the Time PWA.
  ...
  return {
    ...
    transformIndexHtml(): HtmlTagDescriptor[] {
      return [
        ...
        {
          tag: "meta",
          attrs: { name: "apple-mobile-web-app-title", content: "Time" },
          injectTo: "head",
        },
      ];
    },
